package task_manager.api.filter.grammar;
...
import task_manager.api.filter.EqualsFilterCriterion;
import task_manager.api.filter.FilterCriterion;
import task_manager.api.filter.OrFilterCriterion;
...
public class FilterBuilder {
    ...
    private static FilterCriterion buildFilterCriterion(ParseTree tree) {
        if (tree instanceof QueryParser.ExprContext) {
            QueryParser.ExprContext expr = (QueryParser.ExprContext) tree;
            ...
            QueryParser.ConstantContext constant = ((QueryParser.EqualsExprContext) tree).constant();
            if (constant.string() != null) {
                ...
                return new EqualsFilterCriterion(propertyName, propertyValue.substring(1, propertyValue.length()-1));
            ...
            return new ContainsCaseInsensitiveFilterCriterion(propertyName, propertyValue.substring(1, propertyValue.length()-1));
        }
