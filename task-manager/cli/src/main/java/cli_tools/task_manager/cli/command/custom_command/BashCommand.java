...
public class BashCommand extends CustomCommand {
    ...
    private void fillEnv(List<Task> tasks, Map<String, String> env) {
        for (int i = 0; i < tasks.size(); i++) {
            for (var entry : tasks.get(i).getProperties().entrySet()) {
                String propertyName = entry.getKey().toUpperCase();
                env.put("TASK_" + i + "_" + propertyName,  entry.getValue().toString());
            }
        }
    }
